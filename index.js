...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\nPor favor seja ativo e leia as regras`
				...
				teks = `Tchau @${num.split('@')[0]} não volte mais😡🤙`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Preparando, aguade... ⌛',
				success: '✔️ tudo certo✔️',
				error: {
				...
				only: {
					...
					ownerG: '⚠️ Este comando só pode ser usado pela dona do bot! 🤬',
					ownerB: '❌ Este comando só pode ser usado pelo proprietário do bot! ❌',
					admin: '⚠️ Este comando não pode ser usado por membros comuns',
					Badmin: '❌ Este comando só pode ser usado quando eu virar adm porra! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["554891768033@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'nsfwloli':
					...
					break
				case 'hilih':
					if (args.length < 1) return reply('Cadê o texto, diga?')
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/hilih?teks=${body.slice(7)}`, {method: 'get'})
					reply(anu.result)
					break
				case 'yt2mp3':
					if (args.length < 1) return reply('Cadê o link, diga?')
					...
					break
				case 'ytsearch':
					if (args.length < 1) return reply('O que você está procurando? fotos da linda da adm?')
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Onde está o nome de usuário, diga?', text, {quoted: mek})
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('Comando so presta, se você for o dono do bot 😡')
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui @${mentioned[0].split('@')[0]} agora é admin então tomem cuidado😥`, mentioned, true)
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ responder figurinha an ❌')
					reply(mess.wait)
